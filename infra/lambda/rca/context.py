...
def gather(
    ...
    # Task #128: surface chattering meta-alarms from the same window so the
    # narrative can distinguish "real escalating fault" from "noisy/oscillating
    # signal that fired the threshold N times in a few minutes." Scanned from
    # the same events list we already paid the S3 round-trip for — no extra
    # latency. See _extract_chattering() for the detection heuristic.
    chattering = _extract_chattering(events)
    ...
    return {
        ...
        "chattering_signals": chattering,
        "event_window_minutes": EVENT_WINDOW_MIN,
        "telemetry_window_minutes": TELEMETRY_WINDOW_MIN,
    }


def _extract_chattering(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Find any chattering meta-alarms in the recent event stream.

    The alert engine (src/engine/alert_engine.py) emits a meta-alarm with:
      - id beginning with "CHAT-"
      - message containing "chattering"
      - severity "warning"
    when an underlying threshold alarm fires more than CHATTER_THRESHOLD
    times within CHATTER_WINDOW_S. The meta-alarm flows through the same
    MQTT → S3 audit path, so it shows up in `recent_events`.

    Returns a list of compact records the prompt can render — one per
    chattering metric, with enough detail for the LLM to note "this signal
    was thrashing" without flooding the prompt with full alert envelopes.
    """
    chattering: list[dict[str, Any]] = []
    seen_metrics: set[str] = set()  # one record per metric, latest wins
    # Walk newest-last (events arrive in chronological order); iterating
    # reversed means the most recent meta-alarm per metric wins.
    for event in reversed(events):
        payload = event.get("payload", event)
        alert_id = payload.get("id") or event.get("id") or ""
        message = payload.get("message") or ""
        if not (alert_id.startswith("CHAT-") or "chattering" in message.lower()):
            continue
        # Best-effort metric extraction. The alert engine's message format:
        #   "{asset_name}: {metric_label} chattering (N fires in M min) — ..."
        metric = "unknown"
        if ": " in message and " chattering" in message:
            after_colon = message.split(": ", 1)[1]
            metric = after_colon.split(" chattering", 1)[0].strip()
        ...
        chattering.append(
            {
                ...
                "metric": metric,
                "message": message,
                "detected_at": payload.get("detected_at") or event.get("ts"),
                "asset_id": payload.get("asset_id") or event.get("asset_id"),
            }
        )
    # Restore chronological order for the prompt (oldest first reads naturally).
    chattering.reverse()
    return chattering
