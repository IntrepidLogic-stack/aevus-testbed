...
# ── Alarm catalog appendix (Task: cache activation + diagnosis quality) ──
# The full ISA-18.2-grounded alarm catalog ships in the Lambda bundle as
# alarm_catalog.md (copied from docs/AEVUS_ALARM_CATALOG_v1.md at packaging
# time). Appending it serves two purposes:
#   1. The model reasons against the SAME priority bands / deadbands /
#      response expectations the platform ships with — narratives can cite
#      catalog alarm IDs instead of re-deriving thresholds.
#   2. It pushes the static system prefix past the 4096-token prompt-cache
#      minimum, activating the cache_control breakpoint handler.py already
#      sets (~90% cheaper repeated reads during alarm storms).
# Loaded once at cold start; the prefix is byte-identical across warm
# invocations, which is what makes it cacheable. Missing file degrades
# gracefully to the base prompt — never fails the Lambda.
try:
    _ALARM_CATALOG = (Path(__file__).parent / "alarm_catalog.md").read_text(encoding="utf-8").strip()
except OSError:
    _ALARM_CATALOG = ""
...
if _ALARM_CATALOG:
    SYSTEM_PROMPT += (
        "\n\n"
        "═══════════════════════════════════════════════════════════════════\n"
        "AEVUS ALARM CATALOG — authoritative alarm definitions (ISA-18.2)\n"
        "═══════════════════════════════════════════════════════════════════\n"
        "When the triggering alarm matches a catalog entry, cite its alarm ID\n"
        "and priority band in your evidence, and align recommended_action with\n"
        "the catalog's operator response expectation.\n\n" + _ALARM_CATALOG + "\n"
    )
